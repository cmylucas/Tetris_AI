...
def get_peaks(area):
    ...
    for col in range(area.shape[1]):
        if 1 in area[:, col]:
            indices_of_ones = np.where(area[:, col] == 1)[0]
            # print(indices_of_ones)
            if len(indices_of_ones) > 0:
                rightmost_index_of_one = np.max(indices_of_ones)
                p = rightmost_index_of_one+1
            else:
                p = 0
...
def get_holes(peaks, area):
    ...
    for col in range(area.shape[1]):
        ...
        if start == 0:
            ...
            holes.append(np.count_nonzero(area[int(start)::-1, col] == 0))
    return holes

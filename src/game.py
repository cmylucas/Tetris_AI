...
class Game:
    def __init__(self, mode, agent=None):
        ...
        if mode == "greedy":
            ...
            self.ai = Lucas_AI()
        elif mode == "geneticlucas":
            if agent == None:
                with open('geneticmodel.pkl', 'rb') as f:
                    model = pickle.load(f)
                self.ai = model
            else:
                self.ai = agent
        elif mode == 'geneticlucastest':
            self.ai = agent
            self.ai2 = Greedy_AI()
        else:
            self.ai = None
    ...
    def run_no_visual(self):
        ...
        while True:
            ...
            newpiece = deepcopy(self.curr_piece)
            x, piece = self.ai.get_best_move(self.board, self.curr_piece)
            # x1, piece1 = self.ai2.get_best_move(newboard, newpiece)
            self.curr_piece = piece
